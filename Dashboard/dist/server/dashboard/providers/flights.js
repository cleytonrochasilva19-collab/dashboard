...
export async function loadFlights(context) {
    try {
        const token = await fetchAccessToken(context);
        const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
        const states = await fetchJson("https://opensky-network.org/api/states/all", headers ? { headers } : undefined);
        return envelope(token ? "live" : "degraded", token ? "OpenSky Network (authenticated)" : "OpenSky Network (public)", {
        ...
        return fallbackEnvelope("OpenSky Network", { activeFlights: null }, "Flights feed needs OpenSky credentials or a reachable public endpoint.");
    }
}
