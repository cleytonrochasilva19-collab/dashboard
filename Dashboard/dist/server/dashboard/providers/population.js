...
async function fetchIndicator(indicator) {
    const url = `https://api.worldbank.org/v2/country/WLD/indicator/${indicator}` +
        "?format=json&mrv=1";
...
export async function loadPopulation() {
    try {
        ...
        const populationNow = Math.round(population.value + (now.getTime() - baseDate.getTime()) * netPerMs);
        ...
        return fallbackEnvelope("World Bank", {
            populationNow: null,
            birthsToday: null,
            deathsToday: null,
            modelYear: null
